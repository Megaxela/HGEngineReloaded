...
void TestScene::start()
{
    ...
    auto textureImage =
        application()->resourceManager()->load<HG::Utils::STBImageLoader>("Assets/Textures/texture.png");

    // Creating texture
    auto texture = registerResource(new (application()->resourceCache())
                                        HG::Rendering::Base::Texture(textureImage,
        ...
        for (uint32_t iy = 0; iy < 32; ++iy)
        {
            addGameObject(HG::Core::GameObjectBuilder(application()->resourceCache())
                              ...
                              .addBehaviour(new HG::Rendering::Base::Behaviours::Mesh(
                                  cube.guaranteeGet()->children()[0]->meshes()[0], material)));
        }
    }
}

...
public:
     ...
     * @param future Future.
     */
    FutureHandler(std::shared_future<ResultType> future) : m_predefinedValue(), m_future(std::move(future))
    {
    }
     ...
     * @param result Result.
     */
    FutureHandler(ResultType result) : m_predefinedValue(std::move(result)), m_future()
    {
    }

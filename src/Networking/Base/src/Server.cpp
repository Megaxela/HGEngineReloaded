...
struct PosixServerData : public HG::Networking::Base::AbstractInternalData
{
    struct StableConnectionsThreadData
    {
        ...
        std::unordered_map<HG::Networking::Base::LowLevel::Socket, HG::Networking::Base::StableConnection*> connections;
...
{
Server::Server(HG::Core::Application* application) : m_application(application), m_data(nullptr), m_isRunning(false)
{
}
...
void Server::start(std::uint16_t udpPort, std::uint16_t tcpPort, std::size_t stableThreads)
{
    ...
    auto udpAddress =
        HG::Networking::Base::LowLevel::createInternalIPv4Address(HG::Networking::Base::LowLevel::kAnyAddress, udpPort);
    ...
    auto tcpAddress =
        HG::Networking::Base::LowLevel::createInternalIPv4Address(HG::Networking::Base::LowLevel::kAnyAddress, tcpPort);
...
void Server::listenJob()
{
    ...
    while (m_isRunning)
    {
        ...
        {
            auto newConnection =
                HG::Networking::Base::LowLevel::acceptNewConnection(currentServerData->listenTCPSocket);
            ...
            auto unloadedThreadIter =
                std::min_element(currentServerData->stableConnectionDataByThread.begin(),
        ...
        if (HG::Networking::Base::LowLevel::isDescriptorReady(currentServerData->listenUDPSocket, readSet))
        {
            ...
            HG::Networking::Base::LowLevel::readFromUnstableSocket(
                currentServerData->listenUDPSocket, internalAddress, buffer, kMaxUnstablePacketSize);

            auto address = HG::Networking::Base::LowLevel::internalToExternalAddress(internalAddress);

            auto packetHeader = HG::Networking::Base::PacketLayers::UnstablePacketHeader::parse(buffer);
            ...
            HGInfo("Data: {}",
                   (const char*)(buffer.data() + HG::Networking::Base::PacketLayers::UnstablePacketHeader::kSize));
...
void Server::stableConnectionsProcessor(std::uint32_t index)
{
    ...
    while (m_isRunning)
    {
        ...
        applyAtDescriptors(readSet, [&connectionsThreadData](HG::Networking::Base::LowLevel::Socket sock) {
            ...
            if (internalData->state == StableConnectionInternalData::ReadState::Initial) // Packet initial read
            {
                ...
                internalData->lastPacketHeader =
                    HG::Networking::Base::PacketLayers::StablePacketHeader::parse(internalData->buffer);
            ...
            {
                if (!HG::Networking::Base::LowLevel::readFromStableSocket(
                        internalData->socket, internalData->buffer, internalData->lastPacketHeader.dataSize))
                {
                    HGInfo("Connection to {} was closed on remote host", stableConnection->address().toString());
                    stableConnection->close();
                    return;

...
TEST(Core, TransformParentAfterFree)
{
...

    {
        ...
        ASSERT_NE(std::find(parent->children().begin(), parent->children().end(), child), parent->children().end());

        delete child;

        ASSERT_EQ(std::find(parent->children().begin(), parent->children().end(), child), parent->children().end());
    }

...
public:
     ...
     */
    explicit TiledMapCollider(ColliderRemover remover, TiledMapRenderer* renderer) :
        m_remover(remover),
        m_renderer(renderer),
        m_bodies()
    {
    ...
    void clear()
    {
        for (auto&& body : m_bodies)
        {
            if (m_remover)
            {
                m_remover(body, scene()->application()->physicsController());
            }
        }
    }
...
private:
    void proceedGroupLayer(const HG::Standard::Behaviours::TiledMap::Group* group, glm::ivec2 offset)
    {
        for (auto&& layer : group->layers)
        {
            ...
            case TiledMap::Layer::Type::Object:
                proceedObjectLayer(static_cast<TiledMap::ObjectLayer*>(layer), offset + layer->offset);
                break;

            case TiledMap::Layer::Type::Group:
                proceedGroupLayer(static_cast<TiledMap::Group*>(layer), offset + layer->offset);
                break;
    ...
    void proceedObjectLayer(const HG::Standard::Behaviours::TiledMap::ObjectLayer* layer, glm::ivec2 offset)
    {
        auto metersPerPixel = m_renderer->metersPerPixel();

        glm::vec2 worldMapTileSize = {m_renderer->map()->properties().tileSize.x * metersPerPixel,
                                      m_renderer->map()->properties().tileSize.y * metersPerPixel};

        glm::vec2 worldLayerSize = {m_renderer->map()->properties().size.x * worldMapTileSize.x,
        ...
        for (auto&& object : layer->objects)
        {
            ...
            if (object->objectType == TiledMap::ObjectLayer::Object::Type::Rectangle)
            {
                ...
                m_bodies.push_back(m_creator.createRectangle(
            ...
            else if (object->objectType == TiledMap::ObjectLayer::Object::Type::Ellipse)
            {
                ...
                m_bodies.push_back(
                    m_creator.createEllipse(-(worldLayerSize.x / 2) + (offset.x + ellipse->position.x) * metersPerPixel,
            ...
            else if (object->objectType == TiledMap::ObjectLayer::Object::Type::Polygon)
            {
                ...
                {
                    globalPointsOffset.emplace_back(point.x * metersPerPixel, -point.y * metersPerPixel);
                }

                m_bodies.push_back(
                    m_creator.createPolygon(-(worldLayerSize.x / 2) + (offset.x + polygon->position.x) * metersPerPixel,
